...
private function TriggerConditionEnter takes nothing returns boolean
    ...
    if (owner != Player(PLAYER_NEUTRAL_PASSIVE) and owner != Player(PLAYER_NEUTRAL_AGGRESSIVE) and owner != GetMapBossesPlayer() and GetUnitTypeId(GetTriggerUnit()) != 'E008' and GetUnitTypeId(GetTriggerUnit()) != 'E00R' and not UnitHasBuffBJ(GetTriggerUnit(), 'Bvul') and GetUnitAbilityLevel(GetTriggerUnit(), 'Avul') == 0 and not IsUnitInGroup(GetTriggerUnit(), udg_TuskarrQuest3Ships) and GetPlayerTechCountSimple('R029', owner) == 0) then
